...
| GAP-7 | Graph→형제 계약 갱신안(family-synergy.md 등 8건) 작성 완료, 형제 저장소 미반영 | 🟠 대기 | Graph는 읽기전용이라 직접 못 씀. 이 우산 저장소(항목 7번)는 이번에 반영 완료, 나머지 7곳은 **사용자 직접 복사·push 대기**(`SoDam-Graph-Eng/docs/FAMILY_CONTRACT_PROPOSAL.md` 참고) |
...
| Context | 0.1.0 | 1 완료 | ✅ 177 test | AI 안전 작업 소진(2026-08-04 재확인) — 남은 4건 전부 사람 결정(제품명·베타·건강점수 산식·공개여부) |
| Loop | 0.1.0 | 1a 완료·1b 코드완료 | ✅ 178 test | AI 작업 소진(2026-08-04 재확인) — 세션격리 라이브 확인 1건(선택) |
| Agentic | 0.1.0 | init-mvp 완료 | ✅ selftest 22 | Phase 2 PreToolUse 강제 예정·베타 1명·법무·master 정리 |
| Prompt | 0.1.0 | 스킬10종 완료 (웹앱 폐기) | ✅ | 개인 용도로 실사용 검증 완료(2026-07-15) — 공개 계획 없음, 자동테스트 0(정식 CI 미도입) |
| Reverse | 0.1.0 | 1 라이브·2 안드로이드 라이브 | ✅ selftest 8 | 2층 과차단·정규화 배포·독립 검수·법무 |
| Graph | 0.1.0 | Phase 1 진행 (M0·M2 완료) | ✅ M0 8/8·M2 8/8 | 마켓 재설치+재시작 검증(M-PUSH, 사람 몫)·형제 계약 갱신안 8건 push(M-FAMILY-SYNC, 사람 몫)·2주 효과측정(M-OBSERVE, ~2026-08-16) |

---

> 최종 수정: 2026-08-04 (7형제 확장 반영 — Graph 추가, Context/Loop 버전현황을 직접 git·CHECKPOINT 재조사로 갱신) ·
